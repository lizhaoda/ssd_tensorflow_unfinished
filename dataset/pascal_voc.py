...
class PascalVoc(Imdb):
    ...
    def _load_image_labels(self):
        ...
        # find the image_set_index in the trainval.txt
        for idx in self.image_set_index:
            ...
            # find the object in each picture, which is rendered in the xml
            for obj in root.iter('object'):
                difficult = int(obj.find('difficult').text)
                if not self.config['use_difficult'] and difficult == 1:
                    continue
                cls_name = obj.find('name').text
                # very importment, beacuse I reduce the class number from twenty to ten, if a picture has all the object
                # that not in the self.classes, then the label will be []
            ...
            # so each_temp contain the objects in a picture
        ...
        labels = []
        for label in temp:
            # while the label is empty, then continue
            if len(label):
                # print('label',label)
                # print('self.padding-label.shape[0]',self.padding-label.shape[0])
                tmp_label = np.lib.pad(label, ((0, self.padding-label.shape[0]), (0,0)), \
